\documentclass[11pt]{article}
% Packages
\usepackage[T1]{fontenc} % Fontes T1
\usepackage[utf8]{inputenc} % Input UTF8
\usepackage{csquotes}
\usepackage[portuguese]{babel} %Usar língua portuguesa
\usepackage{blindtext}
\usepackage{graphicx}
\usepackage{geometry}
\usepackage{float}
\usepackage{hyperref}
\usepackage{indentfirst}
\usepackage[printonlyused]{acronym}
\usepackage{ragged2e}
\usepackage{textcomp}
\usepackage{multicol}
\usepackage{listings}
\usepackage{color}
\usepackage{fancyhdr}
\usepackage{soul}


\geometry{a4paper,inner=20mm,outer=20mm,top=15mm}

% Define colors
\definecolor{dkgray}{rgb}{0.37,0.37,0.37}
\definecolor{gray}{rgb}{0.5,0.5,0.5}
\definecolor{mauve}{rgb}{0.58,0,0.82}
\definecolor{codegray}{RGB}{234, 234, 234}

\setlength{\parskip}{0.em plus 0.1em minus 0.2em}
\setlength{\headheight}{134pt}

\lstset{frame=tb,
  language=C,
  aboveskip=3mm,
  belowskip=3mm,
  showstringspaces=false,
  columns=flexible,
  basicstyle={\small\ttfamily},
  numbers=none,
  numberstyle=\tiny\color{gray},
  keywordstyle=\color{blue},
  commentstyle=\color{dkgray},
  stringstyle=\color{mauve},
  breaklines=true,
  breakatwhitespace=true,
  tabsize=4
}

% Document
\begin{document}

    % Define keybinds
    \def\title{\textbf{RELATÓRIO DE ANÁLISE DE COMPLEXIDADE}}
    \def\authorjp{João Bastos (113470)}
    \def\authorrg{Rúben Gomes (113435)}
    \def\authors{João Bastos (113470), Rúben Gomes(113435)}
    \def\contacts{(113470) joaop.bastos@ua.pt, (113435) rlcg@ua.pt}
    \def\department{Departamento de Eletrónica, Telecomunicações e Informática}
    \def\university{Universidade de Aveiro}

    \pagestyle{fancy}
    

    % Clear header and footer
    \fancyhead{}\fancyfoot{}

    % Header
    \fancyhead[C]{
        \begin{center}
            \includegraphics{images/ua}\\
            \vspace{2mm}
            \textbf{{\LARGE \title}}\\
            \vspace{2mm}
            % \large Trabalho realizado por: \authors.\\
        \end{center}
    }

    \fancyfoot[R]{\thepage}
    
    \input{sections/introduction.tex}
    
    \input{sections/imagelocate.tex}

    \newpage

    \input{sections/imageblur.tex}   







\end{document}